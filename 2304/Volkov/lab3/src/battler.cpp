...
void poseCallback(const turtlesim::PoseConstPtr& msg){
  ...
  br.sendTransform(tf::StampedTransform(transform_bot1, ros::Time::now(), "world", "Battler"+name));
  tf::Transform knee1;
  knee1.setOrigin( tf::Vector3(1.5, 0, 0.0) );
  q.setRPY(0, 0, -c2);
  knee1.setRotation(q);
  br.sendTransform(tf::StampedTransform(knee1, ros::Time::now(), "Battler"+name, name+"1"));
  tf::Transform knee2;
